...
register int mode;
{
  ...
  mode_buf[0] = (mode & S_IRUSR) ? 'r' : '-';
  mode_buf[1] = (mode & S_IWUSR) ? 'w' : '-';
  mode_buf[2] = (mode & S_IXUSR) ? 'x' : '-';
  mode_buf[3] = (mode & S_IRGRP) ? 'r' : '-';
  mode_buf[4] = (mode & S_IWGRP) ? 'w' : '-';
  mode_buf[5] = (mode & S_IXGRP) ? 'x' : '-';
  mode_buf[6] = (mode & S_IROTH) ? 'r' : '-';
  mode_buf[7] = (mode & S_IWOTH) ? 'w' : '-';
  mode_buf[8] = (mode & S_IXOTH) ? 'x' : '-';
  mode_buf[9] = ' ';
  if (mode & S_ISUID)
	mode_buf[2] = 's';
  if (mode & S_ISGID)
...
#define NNAMES 100
	...
	while (nnames) {
		unsigned int i = nnames >= NNAMES ? NNAMES : nnames;
