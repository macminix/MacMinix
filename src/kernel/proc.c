...
#endif
#if (CHIP == M68000)
FORWARD _PROTOTYPE( void cp_mess, ( int src , struct proc *src_p , message *src_m , struct proc *dst_p , message *dst_m ) );
#define CopyMess(s,sp,sm,dp,dm) \
 ...
 */
  ...
  if (proc_number(caller_ptr) == IDLE && src == ANY) {
    lock_pick_proc();
    if (proc_number(caller_ptr) == IDLE) {
    	/* we have nothing else to do, so run idle again */
    	m_ptr->m_source = HARDWARE;
    	m_ptr->m_type = OK;
    }
  }
