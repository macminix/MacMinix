...
FORWARD _PROTOTYPE( int do_sig, ( message *m_ptr ) );
 ...
 */
  ...
  for (rp = BEG_SERV_ADDR; rp < END_PROC_ADDR; rp++)
	if (rp->p_flags & PENDING) {
		sigm.m_type = KSIG;
		sigm.PROC1 = proc_number(rp);
		sigm.SIG_MAP = rp->p_pending;
		sig_procs--;
		if (lock_mini_send(cproc_addr(HARDWARE), MM_PROC_NR, &sigm) != OK)
...
/* Return the base and size of the next chunk of memory of a given type. */
  ...
  if (beenhere)
	...
	m_ptr->COUNT = mem_bytes >> CLICK_SHIFT;
	beenhere++;
  }
#endif /* (MACHINE == MACINTOSH) */
