...
#define HD_OPEN		0	/* open code OLD WILL GO AWAY */
...
FORWARD _PROTOTYPE( int w_open, ( struct wini *w , char *name ) );
...
/* Main program of the wini disk driver task. */
  ...
  if (BOOT_DEV == DEV_HD0) {
      	setup(&conf, 0, 0);
	override(conf.root);
     	if ((r = w_open(&wini[0], conf.root)) != 0)
  ...
  while (TRUE) {
	...
	switch(w_mess.m_type) {
	    case DISK_READ:
	    case DISK_WRITE:	r = w_do_rdwt(&w_mess, SINGLE);		break;
	    case SCATTERED_IO:	r = w_do_vrdwt(&w_mess); 		break;
	    case DISK_IOCTL:	r = w_do_ioctl(&w_mess);		break;
...
/* Carry out a read or write request from the disk. */
  ...
  w = &wini[device];
  /* check for eof */
  if ((m_ptr->POSITION / BLOCK_SIZE) > w->nblocks)
	return(0);
  pbio = &w->pb.ParamBlkType.ioParam;
  w->pb.ioCompletion = (ProcPtr)w_int;
  pbio->ioRefNum = w->fref;
  pbio->ioActCount = 0;
  ...
  receive(HARDWARE, &m);
  if (w->pb.ioResult != 0 || pbio->ioActCount != m_ptr->COUNT)
    return EIO;
...
PUBLIC void w_close_all()
{
  ...
  for (i = 0, w = &wini[0]; i < NR_DEVICES; i++, w++)
    if (w->fref != 0) {
  	w->pb.ioCompletion = 0L;
  	w->pb.ParamBlkType.ioParam.ioRefNum = w->fref;
  	PBKillIO(&w->pb, 0);
   	PBClose(&w->pb, 0);
  	w->fref = 0;
    }
...
message *m_ptr;			/* pointer to read or write w_message */
{
  ...
  switch (request) {
  	case HD_OPEN:	
		...
		if (w->fref != 0)
		   return(EIO);
		w->fref = m_ptr->TTY_FLAGS;
		w_blocks(w);
		strcpy(w->name, "UNKNOWN");
		break;
  	case HD_CLOSE:  
		w->pb.ioCompletion = 0L;
		w->pb.ParamBlkType.ioParam.ioRefNum = w->fref;
	...
	case HD_GETFILE:
		...
		if (w->fref == 0)
		   return(EIO);
		if (w->name[0] != ':' && index(w->name, ':')) 
			n = w->name; /* already full name */
		else {
  			nbuf[0] = strlen(w->name);
			dst = &nbuf[1];
			sn = w->name;
  			while (*sn) {
    		   	  *dst++ = *sn++;
...
char *name;
{
  ...
  w->pb.ioNamePtr = nbuf;
  w->pb.ioVRefNum = 0;
  w->pb.ParamBlkType.ioParam.ioVersNum = 0;
  w->pb.ParamBlkType.ioParam.ioPermssn = fsRdWrPerm;
  w->pb.ParamBlkType.ioParam.ioMisc = 0;
  err = PBOpen(&w->pb, 0);
  if (err == 0) {
  	w->fref = w->pb.ParamBlkType.ioParam.ioRefNum;
  	w_blocks(w);
	strcpy(w->name, name);
  }
  return (err);
...
register struct wini *w;
{
  ...
  w->pb.ParamBlkType.ioParam.ioMisc = 0;
  PBGetEOF(&w->pb, 0);
  w->nblocks = (long)w->pb.ParamBlkType.ioParam.ioMisc/BLOCK_SIZE;
}
  ...
  for (base = 0; base < nr_requests; base = limit) {
	...
	for (limit = base+1; limit < nr_requests; limit++) {
		if (iovec[limit].io_request == iovec[limit-1].io_request
		 && iovec[limit].io_position 
			== iovec[limit-1].io_position+iovec[limit-1].io_nbytes
