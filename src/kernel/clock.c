...
PUBLIC  time_t pending_ticks;	/* ticks seen by low level only */
...
PRIVATE time_t  mac_time;
PRIVATE char	clock_on = FALSE;
#ifdef THINK_C
PUBLIC long	clk_vec[3];
extern long Time  : 0x020C;
extern long Ticks : 0x016A;
extern long CurrentA5: 0x0904;
#else
PUBLIC long	clk_vec[1];
extern long Time;
extern long Ticks;
extern long CurrentA5;
#endif
#endif
...
#if (MACHINE == MACINTOSH)
FORWARD _PROTOTYPE( time_t convert_time, ( long t ) );
...
#define YEARS 	 (1970-1904)	/* # years between mac and minix */
#define LEAPDAYS ((YEARS-1)/4)	/* # leap days occurred in above years */

/*===========================================================================*
 *				tounixtime				     *
 *===========================================================================*/
PRIVATE time_t convert_time(t)
long t;
{
/* Convert t from a Macintosh time to a MINIX time.
 * t is number of seconds from 00:00:00 Jan 1, 1904 local time,
 * but we want number of seconds from 00:00:00 Jan 1, 1970 GMT.
 */
   time_t tmp1;

   tmp1 = ((YEARS * 365L) + LEAPDAYS + 1) * 24L * 60L * 60L;
   return (t - tmp1);
}
...
PUBLIC void start_clock()
{
  ...
  register long *vec = (long *)0x64;
  
  if (clock_on == TRUE)
	return;
  /* take over the the clock interrupt */
  clk_vec[0] = *vec;
  *vec = (long)clocktick;
#ifdef THINK_C
  vec = (long *)0x40;
 ...
 *				end_clock				     *
 *===========================================================================*/
PUBLIC void end_clock()
{
  /* restore clock vectors */
  register long *vec = (long *)0x64;
...
#endif
#if (MACHINE == MACINTOSH)
  idle_wakeup();		/* possibly wake up event handler */
  tty_wakeup();			/* possibly wake up TTY */
#endif
